...
describe("Shape", function()
	describe("Rectangle", function()
		...
		describe("overlap", function()
			it("should overlap two rectangles", function()
				local r1 = rectangle.new(0, 0, 100, 100)
				local r2 = rectangle.new(20, 20, 50, 50)
				expect(polysec.overlap(r1, r2)).toEqual(true)
			end)

			it("should not overlap two rectangles", function()
				local r1 = rectangle.new(0, 0, 100, 100)
				local r2 = rectangle.new(200, 200, 100, 100)
				expect(polysec.overlap(r1, r2)).toEqual(false)
			end)

			it("should not collide rectangle and polygon", function()
				local poly = polygon.new({ 50, 50 }, { 75, 100 }, { 100, 50 })
				local rect = rectangle.new(0, 0, 100, 100)
				expect(polysec.overlap(poly, rect)).toEqual(true)
			end)

			it("should not collide polygon and rectangle", function()
				local poly = polygon.new({ 50, 50 }, { 75, 100 }, { 100, 50 })
				local rect = rectangle.new(0, 0, 100, 100)
	...
	describe("Circle", function()
		describe("contains", function()
			...
			it(
				"should contain a ponit inside (on the border) a circle",
				function()
					local p = point.new(10, 12)
		...
		describe("overlap", function()
			it("should overlap two circles", function()
				local c1 = circle.new(0, 0, 100)
				local c2 = circle.new(50, 50, 100)
				expect(polysec.overlap(c1, c2)).toEqual(true)
			end)
			it("should not overlap two circles", function()
				local c1 = circle.new(0, 0, 100)
				local c2 = circle.new(1000, 1000, 100)
				expect(polysec.overlap(c1, c2)).toEqual(false)
			end)
		end)
